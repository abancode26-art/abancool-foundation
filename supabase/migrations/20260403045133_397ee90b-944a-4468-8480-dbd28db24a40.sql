...
CREATE OR REPLACE FUNCTION public.sql_mpesa_initiate(user_uuid uuid, invoice_id_input uuid, phone_number_input text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
    ...
    api_status INTEGER;
    result JSON;
    intasend_url TEXT := 'https://api.intasend.com/api/v1/checkout/';
    ...
    callback_url TEXT;
BEGIN
    clean_phone := REPLACE(REPLACE(REPLACE(phone_number_input, ' ', ''), '^0', '254'), '^\+', '');
    ...
    FROM invoices 
    WHERE id = invoice_id_input AND user_id = user_uuid AND status IN ('unpaid', 'pending');
    ...
    FROM payments 
    WHERE invoice_id = invoice_id_input AND status IN ('initiated'::payment_status, 'pending'::payment_status)
    LIMIT 1;
    ...
    INSERT INTO payments (
        ...
        user_uuid, invoice_id_input, 'intasend_mpesa', 'mpesa_stk_push',
    ...
    BEGIN
        ...
        FROM http((
            ...
            ARRAY[
                ...
                http_header('X-IntaSend-Public-Key', intasend_public)
            ],
            'application/json',
            json_build_object(
        ...
        IF api_status = 200 OR api_status = 201 THEN
            UPDATE invoices SET status = 'pending', updated_at = NOW() WHERE id = invoice_id_input;
        END IF;
        ...
        ELSE
            result := json_build_object(
                'success', false,
                'error', 'STK Push failed: ' || COALESCE(api_response->>'message', 'Unknown error'),
