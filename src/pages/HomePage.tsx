...
export default function HomePage() {
  ...
  // Filter to shared hosting for homepage display
  const sharedProducts = products?.filter(p => p.product_type === 'shared_hosting') || [];
  ...
  return (
    <div>
        ...
        <div className="container mx-auto px-4 lg:px-8 py-20 lg:py-32 relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-4xl mx-auto">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-heading text-primary-foreground mb-6 leading-tight">
              Fast, Reliable Hosting<br /><span className="text-gradient-accent">Your Business Deserves</span>
            </h1>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {trustBadges.map((badge, i) => (
              <motion.div key={badge.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
      ...
      <section className="py-20 lg:py-28">
        <div className="container mx-auto px-4 lg:px-8">
          ...
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground">No hosting plans available yet. Check back soon!</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
              {sharedProducts.map((product: any, i: number) => {
                const pricing = product.hosting_product_pricing || [];
                const monthlyPrice = pricing.find((p: any) => p.billing_cycle === 'monthly');
                const features = Array.isArray(product.features_json) ? product.features_json : [];
                return (
                  <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}
                    className={`relative rounded-2xl border p-8 card-hover ${product.is_featured ? 'border-primary bg-primary/[0.02] ring-2 ring-primary/20' : 'border-border bg-card'}`}>
                    {product.is_featured && (
                    ...
                    <div className="mb-6">
                      ...
                      <span className="text-4xl font-bold font-heading ml-1">{monthlyPrice ? Number(monthlyPrice.price).toLocaleString() : '—'}</span>
                    ...
                    <Button asChild className={`w-full h-12 rounded-xl font-semibold ${product.is_featured ? 'btn-primary-glow' : ''}`} variant={product.is_featured ? 'default' : 'outline'}>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {whyChoose.map((item, i) => (
              <motion.div key={item.title} initial={{ opacity: 0, x: i % 2 === 0 ? -20 : 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="rounded-xl border border-border bg-card p-8 card-hover">
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="text-center mb-14"><h2 className="text-3xl lg:text-4xl font-bold font-heading mb-4">Trusted by Businesses</h2></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {testimonials.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="rounded-xl border border-border bg-card p-8">
                ...
                <div><div className="font-semibold">{t.name}</div><div className="text-sm text-muted-foreground">{t.company}</div></div>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            {faqs.map(faq => (
              <details key={faq.q} className="group rounded-xl border border-border bg-card overflow-hidden">
                <summary className="flex cursor-pointer items-center justify-between p-6 font-semibold font-heading hover:bg-muted/30 transition-colors">
                  {faq.q}<ChevronRight className="h-5 w-5 transition-transform group-open:rotate-90 text-muted-foreground" />
                </summary>
      ...
      <section className="hero-gradient py-20">
        <div className="container mx-auto px-4 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" asChild className="btn-accent-glow bg-accent text-accent-foreground hover:bg-accent/90 h-14 px-8 rounded-xl text-base font-semibold"><Link to="/hosting/shared">Get Started Now</Link></Button>
            <Button size="lg" variant="outline" asChild className="h-14 px-8 rounded-xl text-base font-semibold border-primary-foreground/20 text-primary-foreground hover:bg-primary-foreground/10"><Link to="/contact">Talk to Sales</Link></Button>
          </div>
        </div>
      </section>
    </div>
