...
import { useMyTickets } from '@/hooks/useSupabase';
...
export default function ClientTickets() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <Skeleton className="h-48 rounded-xl" /> : !tickets?.length ? (
            ...
            <tbody>
              {tickets.map((t: any) => (
                <tr key={t.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm capitalize hidden md:table-cell">{t.support_departments?.name || '—'}</td>
